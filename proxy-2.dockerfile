...
# Copy the script named start-etcd-2.sh from the host machine to the container's /app directory
COPY start-etcd-2.sh /app/start-etcd-2.sh

# Give execute permission to the start-etcd-2.sh script to allow it to be run
...
# Set the entry point command to run the start-etcd-2.sh script when the container starts
